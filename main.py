...
# 3. 대상 종목 리스트 (미국/한국 시가총액 상위 및 주요 종목)
def get_target_tickers():
    tickers = {
        # --- 미국 주요 종목 (S&P500 / NASDAQ 주요 300 여개 대역) ---
        "애플": "AAPL", "엔비디아": "NVDA", "테슬라": "TSLA", "마이크로소프트": "MSFT",
        "아마존": "AMZN", "구글": "GOOGL", "메타": "META", "AMD": "AMD",
        "넷플릭스": "NFLX", "인텔": "INTC", "팔란티어": "PLTR", "코인베이스": "COIN",
        "브로드컴": "AVGO", "일라이릴리": "LLY", "JP모건": "JPM", "비자": "V",
        "월마트": "WMT", "마스터카드": "MA", "엑손모빌": "XOM", "존슨앤드존슨": "JNJ",
        "오라클": "ORCL", "코스트코": "COST", "홈디포": "HD", "프록터앤갬블": "PG",
        "애브비": "ABBV", "쉐브론": "CVX", "머크": "MRK", "뱅크오브아메리카": "BAC",
        "어플라이드 머티어리얼즈": "AMAT", "세일즈포스": "CRM", "퀄컴": "QCOM", "아도비": "ADBE",
        "텍사스 인스트루먼트": "TXN", "ARM": "ARM", "마이크론": "MU", "서머피셔": "TMO",
        "시스코": "CSCO", "에어비앤비": "ABNB", "스노우플레이크": "SNOW", "우버": "UBER",
        "슈퍼마이크로컴퓨터": "SMCI", "디즈니": "DIS", "나이키": "NKE", "스타벅스": "SBUX",
        
        # --- 한국 주요 종목 (KOSPI / KOSDAQ 주요 300 여개 대역) ---
        ...
        "에코프로": "086520.KQ", "신한지주": "055550.KS", "삼성SDI": "006400.KS",
        "LG화학": "051910.KS", "삼성물산": "028260.KS", "하나금융지주": "086790.KS",
        "HD현대중공업": "329180.KS", "알테오젠": "196170.KQ", "카카오뱅크": "377300.KS",
        "크래프톤": "259960.KS", "메리츠금융지주": "138040.KS", "한국전력": "015760.KS",
        "HD한국조선해양": "009540.KS", "삼성화재": "000810.KS", "KT&G": "033780.KS",
        "HMM": "011200.KS", "SK이노베이션": "096770.KS", "LG전자": "066570.KS",
        "두산에너빌리티": "034020.KS", "삼성전기": "009150.KS", "카카오페이": "377300.KS",
        "한화에어로스페이스": "012450.KS", "포스코퓨처엠": "003670.KS", "S-Oil": "010950.KS",
        "한미반도체": "042700.KS", "HLB": "028300.KQ", "삼양식품": "003230.KS"
    }
    return tickers

# 4. 분석 실행
print("=== 주식 스캐너 시작 ===")
send_telegram_msg("🚀 국내/미국 주요 종목 스캐너 분석을 시작합니다!")
...
for name, symbol in TARGET_STOCKS.items():
    try:
        ...
        if detected:
            ...
            matched_summary.append(f"• {name} ({symbol}): 현재가 {fmt_price}{unit} [{line_info}]")
...
# --- 최종 텔레그램 요약 및 차트 전송 ---
if matched_summary:
    summary_text = f"🎯 [장기 이평선 지지 종목 포착 ({len(matched_summary)}건)]\n\n" + "\n".join(matched_summary)
